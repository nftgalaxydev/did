...
use gid::registry::QueryMsg as RegistryQueryMsg;
use gid::resolver::{AddressResponse, ConfigResponse, ContentHashResponse, TextDataResponse};
...
pub fn set_address(
    ...
    ADDRESSES.save(deps.storage, (node, IntKeyOld::<u64>::from(coin_type)), &address)?;
    Ok(Response::default())
}
...
pub fn query_address(
    ...
    let address = ADDRESSES.load(deps.storage, (node, IntKeyOld::<u64>::from(coin_type)))?;
